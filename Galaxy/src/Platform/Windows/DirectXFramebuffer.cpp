...
namespace Galaxy
{
	...
	void DirectXFramebuffer::Invalidate()
	{
		...
		if (m_Context)
		{
			...
			hr = m_Context->GetDevice()->CreateShaderResourceView(m_CopyTexture.Get(), &sDesc, m_SRV.GetAddressOf());

			if (m_RTV)
				m_RTV.Reset();

			hr = m_Context->GetDevice()->CreateRenderTargetView(m_CopyTexture.Get(), nullptr, m_RTV.GetAddressOf());
	...
	void DirectXFramebuffer::Bind()
	{
		...
		m_Context->GetContext()->OMSetRenderTargets(1, m_RTV.GetAddressOf(), nullptr);
	}
